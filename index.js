...
// استبدل الرابط أدناه برابط الاتصال الخاص بك مع وضع كلمة المرور في المكان المخصص
const MONGO_URI = process.env.MONGO_URI || "mongodb+srv://<username>:<password>@cluster.xxxxx.mongodb.net/albaraa_db?retryWrites=true&w=majority";

mongoose.connect(MONGO_URI)
    .then(() => console.log('✅ تم الاتصال بنجاح بقاعدة بيانات MongoDB Atlas'))
    .catch(err => console.error('❌ خطأ في الاتصال بقاعدة البيانات:', err));

// --- 📂 تعريف النماذج (Schemas & Models) لـ MongoDB ---
...
app.post('/login', (req, res) => {
    ...
    if (password === CORRECT_PASSWORD) {
        res.cookie('albaraa_auth', 'true', { httpOnly: true, maxAge: 24 * 60 * 60 * 1000 });
        return res.redirect('/');
    }
...
app.get('/', async (req, res) => {
    try {
        ...
        const todayCustomerPayments = customerPayments.filter(p => p.date === today).reduce((a, b) => a + (Number(b.amount) || 0), 0);
        
        const todayPurchases = purchases.filter(p => p.date === today).reduce((a, b) => a + (Number(b.total) || 0), 0);
        const todaySupplierPayments = supplierPayments.filter(p => p.date === today).reduce((a, b) => a + (Number(b.amount) || 0), 0);
        
        const todayExpenses = expenses.filter(e => e.date === today).reduce((a, b) => a + (Number(b.amount) || 0), 0);

        const totalCustomerDebts = customers.reduce((acc, c) => {
            const invs = invoices.filter(i => i.customerId == c._id.toString());
            const pays = customerPayments.filter(p => p.customerId == c._id.toString());
        ...
        const totalSupplierDebts = suppliers.reduce((acc, s) => {
            const pur = purchases.filter(p => p.supplierId == s._id.toString());
            const pays = supplierPayments.filter(p => p.supplierId == s._id.toString());
...
app.get('/invoices', async (req, res) => {
    try {
        ...
        let saleList = await Invoice.find();
        let purchaseList = await Purchase.find();
        const customers = await Customer.find();
        const suppliers = await Supplier.find();
        ...
        const saleRows = saleList.map(i => {
            const customer = customers.find(c => c._id.toString() == i.customerId);
            ...
            <tr>
                ...
                <td>
                    <a href="/invoices/view-sale/${i._id}" class="btn" style="padding:4px 8px; font-size:11px;">عرض/طباعة</a>
                    <a href="/invoices/edit-sale/${i._id}" class="btn btn-secondary" style="padding:4px 8px; font-size:11px;">تعديل</a>
                    <a href="/invoices/delete-sale/${i._id}" onclick="return confirm('هل أنت تأكد من حذف فاتورة البيع هذه؟')" class="btn btn-danger" style="padding:4px 8px; font-size:11px;">حذف</a>
        ...
        const purchaseRows = purchaseList.map(p => {
            const supplier = suppliers.find(s => s._id.toString() == p.supplierId);
            ...
            <tr>
                ...
                <td>
                    <a href="/invoices/view-purchase/${p._id}" class="btn" style="padding:4px 8px; font-size:11px;">عرض/طباعة</a>
                    <a href="/invoices/edit-purchase/${p._id}" class="btn btn-secondary" style="padding:4px 8px; font-size:11px;">تعديل</a>
                    <a href="/invoices/delete-purchase/${p._id}" onclick="return confirm('هل أنت تأكد من حذف فاتورة الشراء هذه؟')" class="btn btn-danger" style="padding:4px 8px; font-size:11px;">حذف</a>
                    <a href="${waUrl}" target="_blank" class="btn btn-whatsapp" style="padding:4px 8px; font-size:11px;">📲 واتساب</a>
                </td>
            </tr>
            `;
...
app.get('/invoices/new-sale', async (req, res) => {
    try {
        const customers = await Customer.find();
        const drivers = await Driver.find();
        const vehicles = await Vehicle.find();

        const customerOpts = customers.map(c => `<option value="${c._id}">${c.name} ${c.phone ? '(' + c.phone + ')' : ''}</option>`).join('');
        const driverOpts = drivers.map(d => `<option value="${d.name}">${d.name}</option>`).join('');
        const vehicleOpts = vehicles.map(v => `<option value="${v.number}">${v.number}</option>`).join('');
...
app.post('/invoices/save-sale', async (req, res) => {
    try {
        ...
        await newInv.save();
        res.redirect(`/invoices/view-sale/${newInv._id}`);
    } catch (err) {
        console.error(err);
        res.status(500).send('خطأ في حفظ الفاتورة');
    }
});

// --- تعديل فاتورة بيع ---
app.get('/invoices/edit-sale/:id', async (req, res) => {
    try {
        ...
        const customerOpts = customers.map(c => `<option value="${c._id}" ${c._id.toString() == inv.customerId ? 'selected' : ''}>${c.name} ${c.phone ? '(' + c.phone + ')' : ''}</option>`).join('');
        const driverOpts = drivers.map(d => `<option value="${d.name}" ${d.name === inv.driverName ? 'selected' : ''}>${d.name}</option>`).join('');
        const vehicleOpts = vehicles.map(v => `<option value="${v.number}" ${v.number === inv.vehicleNumber ? 'selected' : ''}>${v.number}</option>`).join('');

        const content = `
            <h2>✏️ تعديل فاتورة بيع (${inv.id})</h2>
            <br>
            <form action="/invoices/update-sale/${inv._id}" method="POST" class="card">
                <div class="form-group">
                    <label>اختر الزبون</label>
                    <select name="customerId" required>
                        <option value="">اختر الزبون المسجل...</option>
...
app.post('/invoices/update-sale/:id', async (req, res) => {
    try {
        const customer = await Customer.findById(req.body.customerId);
        const qty = Number(req.body.qty) || 0;
        const price = Number(req.body.price) || 0;
        ...
        console.error(err);
        res.status(500).send('خطأ في تحديث الفاتورة');
    }
});

// --- حذف فاتورة بيع ---
app.get('/invoices/delete-sale/:id', async (req, res) => {
    try {
...
app.get('/invoices/new-purchase', async (req, res) => {
    try {
        const suppliers = await Supplier.find();
        const supplierOpts = suppliers.map(s => `<option value="${s._id}">${s.name}</option>`).join('');
...
app.post('/invoices/save-purchase', async (req, res) => {
    try {
        ...
        await newPur.save();
        res.redirect(`/invoices/view-purchase/${newPur._id}`);
    } catch (err) {
        console.error(err);
        res.status(500).send('خطأ في حفظ فاتورة الشراء');
    }
});

// --- تعديل فاتورة شراء ---
app.get('/invoices/edit-purchase/:id', async (req, res) => {
    try {
        const pur = await Purchase.findById(req.params.id);
        if (!pur) return res.send('فاتورة الشراء غير موجودة');

        const suppliers = await Supplier.find();
        const supplierOpts = suppliers.map(s => `<option value="${s._id}" ${s._id.toString() == pur.supplierId ? 'selected' : ''}>${s.name}</option>`).join('');

        const content = `
            <h2>✏️ تعديل فاتورة شراء (${pur.id})</h2>
            <br>
            <form action="/invoices/update-purchase/${pur._id}" method="POST" class="card">
                <div class="form-group">
                    <label>اختر المجهز / المعمل</label>
                    <select name="supplierId" required>
                        <option value="">اختر المجهز...</option>
...
app.post('/invoices/update-purchase/:id', async (req, res) => {
    try {
        const supplier = await Supplier.findById(req.body.supplierId);
        const qty = Number(req.body.qty) || 0;
        const price = Number(req.body.price) || 0;
...
// --- حذف فاتورة شراء ---
app.get('/invoices/delete-purchase/:id', async (req, res) => {
    try {
...
app.get('/invoices/view-sale/:id', async (req, res) => {
    try {
        const inv = await Invoice.findById(req.params.id);
        if (!inv) return res.send('الفاتورة غير موجودة');
...
app.get('/invoices/view-purchase/:id', async (req, res) => {
    try {
        const pur = await Purchase.findById(req.params.id);
        if (!pur) return res.send('فاتورة الشراء غير موجودة');

        const supplier = await Supplier.findById(pur.supplierId);
        ...
        const rows = customers.map(c => {
            ...
            return `
                <tr>
                    ...
                    <td>
                        <a href="/customers/account/${c._id}" class="btn" style="padding:4px 8px; font-size:11px;">كشف الحساب</a>
                        <a href="/customers/edit/${c._id}" class="btn btn-secondary" style="padding:4px 8px; font-size:11px;">تعديل</a>
                        <a href="/customers/delete/${c._id}" onclick="return confirm('هل أنت تأكد من حذف هذا الزبون؟')" class="btn btn-danger" style="padding:4px 8px; font-size:11px;">حذف</a>
                    </td>
                </tr>
            `;
        }).join('');

        const customerOptions = customers.map(c => `<option value="${c._id}">${c.name}</option>`).join('');
...
app.get('/customers/edit/:id', async (req, res) => {
    try {
        ...
        const content = `
            ...
            <br>
            <form action="/customers/update/${customer._id}" method="POST" class="card">
                <div class="form-group">
...
app.post('/customers/quick-sale', async (req, res) => {
    try {
        const customer = await Customer.findById(req.body.customerId);
        const qty = Number(req.body.qty) || 0;
        const price = Number(req.body.price) || 0;
        ...
        console.error(err);
        res.status(500).send('خطأ في البيع السريع');
    }
});

app.get('/customers/account/:id', async (req, res) => {
    try {
        const customer = await Customer.findById(req.params.id);
        if (!customer) return res.send('الزبون غير موجود');

        const invs = await Invoice.find({ customerId: customer._id.toString() });
        const pays = await CustomerPayment.find({ customerId: customer._id.toString() });
        ...
        const invRows = invs.map(i => {
            ...
            <tr>
                ...
                <td>${Number(i.paid).toLocaleString()} د.ع</td>
                <td>
                    <a href="/invoices/view-sale/${i._id}" class="btn" style="padding:2px 6px; font-size:11px;">عرض/طباعة</a>
...
app.post('/customers/pay', async (req, res) => {
    try {
        ...
        console.error(err);
        res.status(500).send('خطأ في حفظ الدفعة');
    }
});
        ...
        const rows = suppliers.map(s => {
            ...
            return `
                <tr>
                    ...
                    <td style="color:${rem>0?'var(--danger)':'var(--success)'}; font-weight:bold;">${rem.toLocaleString()} د.ع</td>
                    <td>
                        <a href="/suppliers/edit/${s._id}" class="btn btn-secondary" style="padding:3px 6px; font-size:11px;">تعديل</a>
                        <a href="/suppliers/delete/${s._id}" onclick="return confirm('هل أنت تأكد من حذف هذا المجهز؟')" class="btn btn-danger" style="padding:3px 6px; font-size:11px;">حذف</a>
                    </td>
                </tr>
            `;
        }).join('');
...
app.get('/suppliers/edit/:id', async (req, res) => {
    try {
        ...
        const content = `
            ...
            <br>
            <form action="/suppliers/update/${supplier._id}" method="POST" class="card">
                <div class="form-group">
...
app.post('/suppliers/update/:id', async (req, res) => {
    try {
...
app.get('/suppliers/delete/:id', async (req, res) => {
    try {
...
app.post('/purchases/add', async (req, res) => {
    try {
        const supplier = await Supplier.findById(req.body.supplierId);
        const qty = Number(req.body.qty) || 0;
        const price = Number(req.body.price) || 0;
        ...
        console.error(err);
        res.status(500).send('خطأ في الشراء السريع');
    }
});
    ...
    try {
        ...
        const driverRows = drivers.map(d => `
            <tr>
                ...
                <td>${d.phone || '—'}</td>
                <td>${d.notes || '—'}</td>
                <td><a href="/drivers/delete/${d._id}" class="btn btn-danger" style="padding:3px 6px; font-size:11px;">حذف</a></td>
            </tr>
        `).join('');
        ...
        const vehicleRows = vehicles.map(v => `
            <tr>
                <td><b>${v.number}</b></td>
                <td>${v.notes || '—'}</td>
                <td><a href="/vehicles/delete/${v._id}" class="btn btn-danger" style="padding:3px 6px; font-size:11px;">حذف</a></td>
            </tr>
        `).join('');
...
app.post('/drivers/add', async (req, res) => {
    try {
        await Driver.create({ name: req.body.name, phone: req.body.phone, notes: req.body.notes });
        res.redirect('/fleet');
    } catch (err) {
        console.error(err);
        res.status(500).send('خطأ في الحفظ');
...
app.get('/drivers/delete/:id', async (req, res) => {
    try {
...
app.post('/vehicles/add', async (req, res) => {
    try {
        await Vehicle.create({ number: req.body.number, notes: req.body.notes });
        res.redirect('/fleet');
    } catch (err) {
        console.error(err);
        res.status(500).send('خطأ في الحفظ');
...
app.get('/vehicles/delete/:id', async (req, res) => {
    try {
    ...
    try {
        ...
        const rows = employees.map(e => `
            <tr>
                ...
                <td>
                    <a href="/employees/edit/${e._id}" class="btn btn-secondary" style="padding:3px 6px; font-size:11px;">تعديل</a>
                    <a href="/employees/delete/${e._id}" class="btn btn-danger" style="padding:3px 6px; font-size:11px;">حذف</a>
                </td>
            </tr>
        `).join('');
...
app.get('/employees/edit/:id', async (req, res) => {
    try {
        ...
        const content = `
            ...
            <br>
            <form action="/employees/update/${emp._id}" method="POST" class="card">
                <div class="form-group">
    ...
    try {
        ...
        const rows = expenses.map(e => `
            <tr>
                ...
                <td>${e.notes || '—'}</td>
                <td>
                    <a href="/expenses/edit/${e._id}" class="btn btn-secondary" style="padding:3px 6px; font-size:11px;">تعديل</a>
                    <a href="/expenses/delete/${e._id}" onclick="return confirm('هل أنت تأكد من حذف هذا المصروف؟')" class="btn btn-danger" style="padding:3px 6px; font-size:11px;">حذف</a>
                </td>
            </tr>
        `).join('');
...
app.get('/expenses/edit/:id', async (req, res) => {
    try {
        ...
        const content = `
            ...
            <br>
            <form action="/expenses/update/${exp._id}" method="POST" class="card">
                <div class="form-group">
                    <label>التصنيف</label>
...
app.post('/expenses/update/:id', async (req, res) => {
    try {
...
app.get('/reports', async (req, res) => {
    try {
        ...
        if (period === 'daily') {
            ...
            filteredPurchases = purchases.filter(p => p.date === today);
            filteredExpenses = expenses.filter(e => e.date === today);
            filteredCustomerPayments = customerPayments.filter(p => p.date === today);
            filteredSupplierPayments = supplierPayments.filter(p => p.date === today);
        } else if (period === 'yearly') {
            filteredInvoices = invoices.filter(i => i.date && i.date.startsWith(currentYear));
            filteredPurchases = purchases.filter(p => p.date && p.date.startsWith(currentYear));
            filteredExpenses = expenses.filter(e => e.date && e.date.startsWith(currentYear));
            filteredCustomerPayments = customerPayments.filter(p => p.date && p.date.startsWith(currentYear));
            filteredSupplierPayments = supplierPayments.filter(p => p.date && p.date.startsWith(currentYear));
        } else {
            filteredInvoices = invoices.filter(i => i.date && i.date.startsWith(currentMonth));
            filteredPurchases = purchases.filter(p => p.date && p.date.startsWith(currentMonth));
            filteredExpenses = expenses.filter(e => e.date && e.date.startsWith(currentMonth));
            filteredCustomerPayments = customerPayments.filter(p => p.date && p.date.startsWith(currentMonth));
            filteredSupplierPayments = supplierPayments.filter(p => p.date && p.date.startsWith(currentMonth));
        }
...
app.get('/reports/export-csv', async (req, res) => {
    try {
        ...
        console.error(err);
        res.status(500).send('خطأ في تصدير الملف');
    }
});
